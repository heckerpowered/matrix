...
sealed interface CasterContext {
    ...
    companion object {
        fun fromEntity(entity: LivingEntity): CasterContext = when (entity) {
            is ServerPlayerEntity -> PlayerCaster(entity)
...
/**
 * Attempts to cast the [CasterContext] to a [HasEntity] and then to a [ServerPlayerEntity].
 *
 ...
 * the entity to a [ServerPlayerEntity]. If the cast is successful, it returns the [ServerPlayerEntity];
 * otherwise, it returns null.
 *
 * @return The [ServerPlayerEntity] if the context is associated with a player, or null if not.
 */
fun CasterContext.asPlayerOrNull(): ServerPlayerEntity? = (this as? HasEntity)?.entity as? ServerPlayerEntity
...
fun CasterContext.tryResolve(): CasterContext {
    ...
    return when (val entity = world.entityLookup.get(ownerUuid)) {
        is ServerPlayerEntity -> PlayerCaster(entity)
        is LivingEntity -> EntityCaster(entity)
        else -> DetachedCaster(world, ownerUuid)
    }
}
 ...
 */
data class PlayerCaster(val player: ServerPlayerEntity) : CasterContext, HasEntity {
    override val entity get() = player
    override val world get() = player.world!!
}
