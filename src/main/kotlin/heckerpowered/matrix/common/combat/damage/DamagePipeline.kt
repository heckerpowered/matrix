...
object DamagePipeline {
    ...
    fun realization(context: DamageRealizationContext) {
        RuleRegistry.all<DamageRealizationRule>()
            .forEach { it.onRealization(context) }
    }

    @JvmStatic
    fun outcome(context: DamageOutcomeContext) {
        RuleRegistry.all<DamageOutcomeRule>()
            .forEach { it.onOutcome(context) }
    }

    @JvmStatic
    fun settlement(context: DamageSettlementContext) {
        RuleRegistry.all<DamageSettlementRule>()
            .forEach { it.onSettlement(context) }
    }
}
