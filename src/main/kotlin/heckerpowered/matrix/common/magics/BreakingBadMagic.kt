...
object BreakingBadMagic : Magic(MatrixLanguage.magicBreakingBad, 10, MatrixLanguage.magicBreakingBadDescription, 10) {
    override fun cast(player: ServerPlayerEntity?, target: LivingEntity, sequence: ChannelSequence) {
        target.addStatusEffect(StatusEffectInstance(StatusEffects.POISON, 20 * 10, 4))
        target.addStatusEffect(StatusEffectInstance(StatusEffects.BLINDNESS, 20 * 10, 4))
