...
val sequences = target.getChannelQueues()
...
fun performChannel() {
channelQueue.magics.add(ChannelingMagic(magic,, 0, data))
...
return target.getChannelQueues()[player.uuid]
}
...
.toMutableList()
entity.getChannelQueues().compute(playerUUID) { _, _ ->
ChannelQueue(
...
for (channelingSequence in entity.getChannelQueues()) {
...
for (sequence in entity.getChannelQueues()) {
sequence.value.tick()
}
}
}
...
val channelSequence = (this as MatrixLivingEntity).getChannelQueues()
