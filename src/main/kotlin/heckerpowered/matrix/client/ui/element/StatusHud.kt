...
object StatusHud {
    ...
    private fun renderPhaseWalk(drawContext: DrawContext, tickCounter: RenderTickCounter) {
        ...
        val isPhaseWalking = chestplate.components.getOrDefault(BORROWED_TIME_STATE, false)
        val currentCharge = chestplate.components.getOrDefault(BORROWED_TIME_CHARGE, 0L)
        val maxCharge = chestplate.components.getOrDefault(BORROWED_TIME_MAX_CHARGE, 0L)
        if (isPhaseWalking) {
