...
vec3 curlNoise(vec3 p) {
    ...
    float dx = snoise3(vec3(p.x + e, p.y, p.z)) - snoise3(vec3(p.x - e, p.y, p.z));
    float dy = snoise3(vec3(p.x, p.y + e, p.z)) - snoise3(vec3(p.x, p.y - e, p.z));
    float dz = snoise3(vec3(p.x, p.y, p.z + e)) - snoise3(vec3(p.x, p.y, p.z - e));

    float dyz = (dz - dy) / (2.0 * e);
    float dzx = (dx - dz) / (2.0 * e);
    float dxy = (dy - dx) / (2.0 * e);

    return normalize(vec3(dyz, dzx, dxy));
}
...
void main()
{
    vec3 position = vec3(Position, 0.0);
