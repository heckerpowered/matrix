...
object Vector3fStreamCodec : StreamCodec<ByteBuf, Vector3f> {
    override fun encode(output: ByteBuf, value: Vector3f) {
        output.writeFloat(value.x)
        output.writeFloat(value.y)
        output.writeFloat(value.z)
    }

    override fun decode(input: ByteBuf): Vector3f {
        return Vector3f(input.readFloat(), input.readFloat(), input.readFloat())
    }
}