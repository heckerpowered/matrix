...
class MagicLightningBolt(entityType: EntityType<MagicLightningBolt>, level: Level) : LightningBolt(entityType, level) {
    ...
    private fun onStruckByLightning(entity: Entity) {
        // Strikes only happen on the logical server; 26.2 splits Entity#hurt into
        // hurtServer/hurtClient, and hurtServer (normal invulnerability frames) is the
        // exact equivalent of the 1.21 Entity#damage these calls used.
        ...
        when (lightningType) {
            ...
            RED -> entity.hurtServer(serverLevel, damageSource, 20.0F)
            ORANGE -> {
            ...
            PURPLE -> {
                entity.hurtServer(serverLevel, damageSource, 5F)
                level().explode(entity, damageSource, damageCalculator, entity.x, entity.y, entity.z, 1.0F, false, Level.ExplosionInteraction.MOB)
